...
function getFirstVisitGreeting(idol: IdolMeta): string {
  ...
  const greetings = [
    `어? 밈챗 처음이야? 여기서 팬들이랑 직접 얘기할 수 있어서 좋아~ 근데 이름이 뭐야? 😊`,
    `안녕! 밈챗 처음 온 거야? 여기서 나랑 1:1로 대화할 수 있어~ 뭐라고 불러줄까?✨`,
    `오 새로운 얼굴이네! 반가워~ 여기 밈챗이야, 나랑 직접 대화할 수 있어 ㅎㅎ 이름이 뭐야?`,
    `어, 밈챗 처음이지? 여기서 팬들이랑 직접 대화할 수 있어서 나도 좋아~ 뭐라고 부를까?`,
  ];
  return greetings[Math.floor(Math.random() * greetings.length)]!;
}
