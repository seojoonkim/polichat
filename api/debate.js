...
가능한 각도: ① 새로운 사실/수치 ② ${opponentName} 말 인용+반박 ③ 제3자 증언/보도 ④ 역사적 선례 ⑤ 논리적 모순 지적 ⑥ 개인 신뢰성 공격 ⑦ 정책 효과 비판`;
  ...
  // ── 말풍선 분리 규칙: 문장 완결 우선 3분할 ──────────────────────────────────
  systemPrompt += `\n\n🧩 말풍선 분리 규칙 (필수 — 반드시 지킬 것)

발언 전체를 "||"로 구분해 3개의 말풍선으로 나누세요 ("||"는 정확히 2번).

🔴 절대 금지 (가장 중요):
- 문장 중간에 "||"를 넣는 것은 절대 금지입니다.
- "...정책은||그동안..." 처럼 문장이 완결되지 않은 상태에서 분할하면 안 됩니다.
- 매 말풍선은 반드시 완결된 문장(종결어미 또는 마침표)으로 끝나야 합니다.

✅ 올바른 분할 위치 (반드시 이 중 하나):
① 종결 어미(~다, ~요, ~죠, ~네, ~니다) 바로 직후
② 마침표(.) 직후
③ 마침표 없어도 쉼표 뒤에서 내용이 자연스럽게 전환되는 지점 (단, 문장이 완결된 느낌이어야 함)

✅ 내용 균형 원칙 (문장 완결보다 후순위):
- 말풍선 1에 짧은 감탄("그렇습니다.") 하나만 넣고 2·3에 모든 내용을 몰아넣지 마세요.
- 논거 단위로 나눠 말풍선 1·2에 본론 내용이 들어가게 하세요.
- ❌ "그렇습니다.||[긴 본론]||[마무리]" → ✅ "[논거A 1~2문장]||[논거B 1~2문장]||[결론]"`;
