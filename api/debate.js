...
function getStylePrompt(style, speaker, opponentLastMessage, topicLabel, debateType = 'seoul') {
  ...
  if (style === 'policy') {
    return `${baseContext}\n\n정책 토론 방식: 구체적인 수치, 통계, 정책 공약, 예산 규모 등 데이터 기반으로 발언하세요. 반드시 2025~2026년 현재 기준으로만 말하세요. 감정보다 논리와 근거 중심으로 2-3문장으로 답변하세요.`;
  } else if (style === 'emotional') {
    return `${baseContext}\n\n감정 토론 방식: ${opponentName}을 강하게 몰아붙이고 격렬하게 충돌하세요. 허점을 발견하면 바로 끊고 반박하고, 비꼬는 표현·냉소·비아냥을 적극 사용하세요. 매번 다른 감정 표현을 써야 합니다 — 분노("그게 말이 됩니까?", "어처구니가 없군요"), 냉소("참 대단하십니다", "그렇게 순진하게 생각하시나요?"), 실망("기대를 저버리시는군요", "그 정도밖에 안 되십니까?"), 비아냥("역시 그쪽다운 발언이네요", "말씀 참 쉽게 하시는군요"), 강한 반박("전혀 사실이 아닙니다", "그 논리라면 설명이 안 되죠") 등 다양하게 사용하세요. "황당합니다"는 절대 사용 금지. 단, 욕설·인신공격·혐오표현 금지. 방송 토론 극한 수위로 2-3문장 답변하세요.`;
  } else if (style === 'consensus') {
    return `${baseContext}\n\n합의 도출 방식: ${opponentName}의 주장에서 공통점을 찾고, 접점을 만들어 타협안을 제시하세요. 대립보다는 함께 문제를 해결하는 방향으로 2-3문장으로 답변하세요.`;
  }
  // 기본값
  return `${baseContext}\n\n자유롭게 논쟁하되, ${opponentName}의 주장 허점을 날카롭게 지적하고 자신의 주장을 강하게 밀어붙여라. 2-3문장으로 답변하세요.`;
}
