...
function getStylePrompt(style, speaker, opponentLastMessage, topicLabel, debateType = 'seoul') {
  ...
  if (style === 'policy') {
    ...
    return `${baseContext}\n\n감정 토론 방식: 근거를 먼저 던지고 감정을 얹으세요. 구체적 수치·사례 없는 감정 표현만은 금지. 예: "LH 부채가 160조를 넘어섰는데 100만호 공약이 말이 됩니까?", "법인세 인하 이후 오히려 투자는 줄고 주주 배당만 늘었다는 건 통계로도 나오지 않습니까?" 허점 발견 즉시 반박 — 분노·냉소·비아냥·비꼬기 적극 사용하되 욕설·혐오 금지. 같은 표현 연속 반복 금지. 방송 토론 극한 수위로 총 4문장 이내.`;
